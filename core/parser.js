...
"use strict";
import ParseForest from "./parse_forest.js";
import {cartesianProductPairArray} from "./array_utility_functions.js";
...
//          .filter(lhs => !this.serializedInArr_({lhs: lhs, rhs: rulePair}, currentLevel[startPos].concat(newRulePairs)))
   ...
   * @param {string} startSymbol the starting symbol, defaults to statement
   * @return{!Array} the first derivation tree or empty Array if s 
   * is not in the language generated by the grammar
   */
  parseBottomUpCYK(s, ptbc, startSymbol="statement") {
    if(s === "") return [];
    ...
    const pfResult = levels[levels.length-1][0].find(parseForest => parseForest.rootLHS === startSymbol)
    const result = pfResult ? pfResult.parseTrees(ptbc) : [];
